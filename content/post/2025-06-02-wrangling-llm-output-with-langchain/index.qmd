...
LangChain does this by chaining GenAI prompts together with plain Python. The mindset here is: _I don’t trust raw LLM output — how do I mitigate that uncertainty?_
...
No matter how much I tweak the prompt, the LLM _will_ still sometimes return a title that’s too long. Don't waste too much time trying to get the perfect prompt here. It’s better to ask the LLM to fix its own mistakes.
...
There's a final step, which doesn't involve an LLM at all. If the output is _still_ greater than 50 characters, just truncate the last few words. We also trim any trailing punctuation.
...
LangChain works with Pydantic for enforcing a _structured output_ on the LLM. First I'll define my list of countries. For now I'll just consider Australia and the United States, with an "Unknown" option for when the LLM isn't sure. The "Unknown" cards can then be picked up later by a human being and properly classified.
...
Unit tests are a challenge for LLMs, as we're the output is non-deterministic. We're effectively testing a _distribution_ of responses. Traditional unit tests, on the other hand, assume that the same input should always produce the same output.
...
* Remove the test case and _accept_ the incorrect results. This is just for a Kanban intake, after all, so the world won't end if a card is miscategorised regardless of what your product manager tells you.
...
* Use a bigger model. I'm using "gpt-4.1-mini", but I could switch that to "gpt-4.1". Anecdotally, that seems to fix _both_ of the failing tests above, but the cost is roughly 5 times that of the mini model.
